...
(function () {
    ...
    function buildPuzzle() {
        ...
        displayOrder.forEach((originalIndex, position) => {
            fragmentLetterMap[originalIndex] = FRAGMENT_LETTERS[position % FRAGMENT_LETTERS.length];
        });
    ...
    function renderPuzzleList() {
        ...
        displayOrder.forEach((originalIndex) => {
            ...
            const letter = fragmentLetterMap[originalIndex] || '?';

            li.innerHTML =
                '<span class="drag-handle" aria-hidden="true">&#8942;&#8942;</span>' +
                '<span class="fragment-badge">' + letter + '</span>' +
                '<button type="button" class="fragment-play-btn" aria-label="Odtwórz fragment ' + letter + '">' +
                '<span class="play-icon" aria-hidden="true"></span></button>' +
                '<span class="fragment-label">Fragment ' + letter + '</span>' +
