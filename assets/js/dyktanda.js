/* ksza.pl - dyktanda muzyczne (puzzle), ze słuchu.
   Lista dyktand: ../dyktanda/puzzle/dyktanda.json (manifest - patrz opis.txt).
   Silnik: assets/js/puzzle-engine.js. */
(function () {
    KszaPuzzleEngine.init({
        notation: false,
        manifestUrl: '../dyktanda/puzzle/dyktanda.json',
        fileBaseUrl: '../dyktanda/puzzle/',
        labels: {
            minFragments: 'Dyktando musi mieć co najmniej 2 takty z nutami, żeby dało się ułożyć puzzle.',
            manifestWarn: 'Nie udało się wczytać listy dyktand (dyktanda.json):',
            emptyOption: 'Brak dostępnych dyktand',
            emptyHint: 'Nie dodano jeszcze żadnego dyktanda - patrz dyktanda/puzzle/dyktanda.json.',
            loading: 'Wczytywanie dyktanda...',
            loadErrorConsole: 'Błąd wczytywania dyktanda:',
            loadErrorPrefix: 'Błąd wczytywania dyktanda: '
        }
    });
})();
